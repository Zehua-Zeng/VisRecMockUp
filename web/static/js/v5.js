...
var relatedImgLeft = document.querySelector(".relatedImgLeft");
var relatedImgRight = document.querySelector(".relatedImgRight");
...
function generateInitRecPlots() {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      initRec = response.recVegalite;
      // console.log(initRec);
      for (var i = 0; i < initRec.length; i++) {
        for (let prop in initRec[i]) {
          ...
          let VlSpec = initRec[i][prop];
          vegaEmbed(`#${prop_str}`, VlSpec);
          queryMap[prop_str] = VlSpec;
        }
      }
      document.querySelector(".initial_area").style.display = "block";
      document.querySelector(".split_area").style.display = "none";
      document.querySelector(".page_content").style.overflow = "scroll";
      chart_btns();
    },
  });
}
  ...
  $.ajax({
    ...
    success: function (response) {
      if (response.status === "success") {
        ...
        bfsRec = response.bfsRecVegalite;
        dfsRec = response.dfsRecVegalite;
...
function generateRecPlot() {
  ...
  console.log(dfsRec);
  // relatedImg.innerHTML = "";
  relatedImgRight.innerHTML = "";
  relatedImgLeft.innerHTML = "";
  document.querySelector(".initial_area").style.display = "none";
  document.querySelector(".loadmoreDivLeft").style.display = "none";
  document.querySelector(".loadmoreDivRight").style.display = "none";
  document.querySelector(".split_area").style.display = "block";
  document.querySelector(".page_content").style.overflow = "visible";

  let bfsMaxNum = 5;
  if (bfsMaxNum > bfsRec.length) {
    bfsMaxNum = bfsRec.length;
  }

  let dfsMaxNum = 5;
  if (dfsMaxNum > dfsRec.length) {
    dfsMaxNum = dfsRec.length;
  }
  ...
  for (var i = 0; i < bfsRec.length; i++) {
    for (let prop in bfsRec[i]) {
      let prop_str = JSON.stringify(prop).replace(/\W/g, "");
      prop_str += "BFS";
      relatedImgLeft.innerHTML += `<div class='view_wrapper ${prop_str}_wrapper'><i class='fas fa-bookmark add_bm' added="false"></i><i class="fas fa-list-alt specify_chart"></i><div class="views cate" id='${prop_str}'></div></div>`;
  ...
  for (var i = 0; i < dfsRec.length; i++) {
    for (let prop in dfsRec[i]) {
      let prop_str = JSON.stringify(prop).replace(/\W/g, "");
      prop_str += "DFS";
      relatedImgRight.innerHTML += `<div class='view_wrapper ${prop_str}_wrapper'><i class='fas fa-bookmark add_bm' added="false"></i><i class="fas fa-list-alt specify_chart"></i><div class="views cate" id='${prop_str}'></div></div>`;
  ...
  if (bfsRec.length > 5) {
    curBFSRecLen = 5;
    document.querySelector(".loadmoreDivLeft").style.display = "block";
    document
      .querySelector("#loadmoreBtnLeft")
      .addEventListener("click", loadMoreRecLeft);
  }

  if (dfsRec.length > 5) {
    curDFSRecLen = 5;
    document.querySelector(".loadmoreDivRight").style.display = "block";
    document
      .querySelector("#loadmoreBtnRight")
      .addEventListener("click", loadMoreRecRight);
  }
}

function loadMoreRecLeft() {
  console.log("click loadmore left.");
  let maxNum = curBFSRecLen + 5;
  if (maxNum >= bfsRec.length) {
    maxNum = bfsRec.length;
    document.querySelector(".loadmoreDivLeft").style.display = "none";
  }
...
function loadMoreRecRight() {
  console.log("click loadmore right.");
  let maxNum = curDFSRecLen + 5;
  if (maxNum >= dfsRec.length) {
    maxNum = dfsRec.length;
    document.querySelector(".loadmoreDivRight").style.display = "none";
  }
  for (var i = curDFSRecLen; i < maxNum; i++) {
    for (let prop in dfsRec[i]) {
      let prop_str = JSON.stringify(prop).replace(/\W/g, "");
      prop_str += "DFS";
      document.querySelector(`.${prop_str}_wrapper`).style.display = "block";
